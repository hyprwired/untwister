/*
    ...
    Heavily based off GlibcRand.cpp (Copyright Bishop Fox 2014)
...
PHP5_rand::PHP5_rand()
{
    ...
    m_LSBMap.resize(PHP5_RAND_STATE_SIZE);
}

PHP5_rand::~PHP5_rand() {}

const std::string PHP5_rand::getName()
{
    return PHP5_RAND;
}

void PHP5_rand::seed(int64_t long_seed)
{
...
int64_t PHP5_rand::getSeed()
{
    return seedValue;
}

uint32_t PHP5_rand::random()
{
    int32_t result;
    ...
    if (m_isBounded) {
        /*
            ...
            int rand([int min, int max]) uses RAND_RANGE():
                https://github.com/php/php-src/blob/PHP-5.6.27/ext/standard/rand.c#L289
                ...
                if (argc == 2) {
        ...
        */
        result = (uint32_t)((m_minBound) + (long) ((double) ( (double) (m_maxBound) - (m_minBound) + 1.0) * ((result) / ((2147483647) + 1.0))));
    }

    return result;
}

uint32_t PHP5_rand::getStateSize(void)
{
    return PHP5_RAND_STATE_SIZE;
}

void PHP5_rand::setState(std::vector<uint32_t> inState)
{
    m_state = inState;
    m_state.resize(PHP5_RAND_STATE_SIZE, 0);
...
std::vector<uint32_t> PHP5_rand::getState(void)
{
    return m_state;
}

void PHP5_rand::setEvidence(std::vector<uint32_t> evidence)
{
    m_evidence = evidence;
}

std::vector<uint32_t> PHP5_rand::predictForward(uint32_t length)
{
    std::vector<uint32_t> running_state = m_state;
    running_state.resize(PHP5_RAND_STATE_SIZE + length);
...
std::vector<uint32_t> PHP5_rand::predictBackward(uint32_t length)
{
    std::vector<uint32_t> running_state = m_state;
    ...
    /* There is a more memory efficient way to do this. With a
        linked list for the state. But meh.  */
    for(uint32_t i = PHP5_RAND_STATE_SIZE; i < length + PHP5_RAND_STATE_SIZE; i++)
    {
...
/* We just have to make some guesses about the LSBs and then test those
    guesses one by one */
void PHP5_rand::tune_repeatedIncrements()
{
    ...
    while(keepGoing)
    {
        ...
        /* Foreach state integer, test if we can improve predictions by incrementing
            that state value.*/
        for(uint32_t i = 0; i < PHP5_RAND_STATE_SIZE; i++)
        {
            /* Get the success rate of this state */
            std::vector<uint32_t> guesses = this->predictForward(m_evidence.size() - PHP5_RAND_STATE_SIZE);

            int64_t sum = 0;
            for(uint32_t j = 0; j < guesses.size(); j++)
            {
                sum += std::min(guesses[j] - m_evidence[PHP5_RAND_STATE_SIZE + j],
                    m_evidence[PHP5_RAND_STATE_SIZE + j] - guesses[j]);
            }
            ...
            guesses = this->predictForward(m_evidence.size() - PHP5_RAND_STATE_SIZE);

            int64_t sum_new = 0;
            for(uint32_t j = 0; j < guesses.size(); j++)
            {
                /* The guess can NEVER be bigger than the evidence */
                if(guesses[j] > m_evidence[PHP5_RAND_STATE_SIZE + j])
                {
                    sum_new = -1;
                    break;
                }
                sum_new += std::min(guesses[j] - m_evidence[PHP5_RAND_STATE_SIZE + j],
                    m_evidence[PHP5_RAND_STATE_SIZE + j] - guesses[j]);
            }
...
bool PHP5_rand::setLSB(uint32_t index, uint32_t value)
{
    if(m_LSBMap[index].m_isKnown == false)
    {
        if(index < PHP5_RAND_STATE_SIZE)
        {
...
void PHP5_rand::setLSBxor(uint32_t index1, uint32_t index2)
{
...
void PHP5_rand::setLSBor(uint32_t index1, uint32_t index2)
{
...
bool PHP5_rand::handleRemainder(uint32_t i, std::vector<uint32_t> guesses)
{
    /* Did we learn any new information? */
    bool ret = false;

    uint32_t guess = guesses[i];
    uint32_t observed = m_evidence[PHP5_RAND_STATE_SIZE + i];
    uint32_t diff = observed - guess;

    uint32_t diff_first, diff_second;
    /* Get Diff for first predecessor */
    if((i+1) >= PHP5_RAND_STATE_SIZE)
    {
    ...
    if((i+29) >= PHP5_RAND_STATE_SIZE)
    {
        diff_second = m_evidence[PHP5_RAND_STATE_SIZE + i - 3] - guesses[i-3];
    }
    else
    {
        /* All diffs within the first 32 are "0"  */
...
void PHP5_rand::tune_chainChecking()
{
    bool keepGoing = true;
    while(keepGoing)
    {
        keepGoing = false;
        std::vector<uint32_t> guesses = this->predictForward(m_evidence.size() - PHP5_RAND_STATE_SIZE);
        m_LSBMap.resize(PHP5_RAND_STATE_SIZE + guesses.size());

        for(uint32_t i = 0; i < guesses.size()-3; i++)
        {
            uint32_t diff = m_evidence[PHP5_RAND_STATE_SIZE + i] - guesses[i];
            uint32_t diff_next = m_evidence[PHP5_RAND_STATE_SIZE + i + 3] - guesses[i+3];

            if(diff_next - diff == 1)
            {
                keepGoing |= setLSB(i+4, 1);
                keepGoing |= setLSB(PHP5_RAND_STATE_SIZE + i, 1);
            }
            if(diff_next - diff == 0)
            {
...
bool PHP5_rand::isInitState(std::deque<uint32_t> *tmp_state)
{
    return false;
}

bool PHP5_rand::reverseToSeed(int64_t *outSeed, uint32_t depth)
{
    ...
    tmp_state.resize(PHP5_RAND_STATE_SIZE);
    for(uint32_t i = 0; i < PHP5_RAND_STATE_SIZE; i++)
    {
...
void PHP5_rand::tune(std::vector<uint32_t> evidenceForward, std::vector<uint32_t> evidenceBackward)
{
    tune_chainChecking();
    tune_repeatedIncrements();
}

void PHP5_rand::setBounds(uint32_t min, uint32_t max)
{
    m_minBound = min;
    m_maxBound = max;
    m_isBounded = true;
...
int64_t PHP5_rand::getMinSeed()
{
    return 0;
}

int64_t PHP5_rand::getMaxSeed()
{
    return UINT_MAX;
}
